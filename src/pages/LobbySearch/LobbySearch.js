...
import { IonBackButton, IonButton, IonButtons, IonCard, IonCardContent, IonCardHeader, IonCardTitle, IonCol, IonContent, IonGrid, IonHeader, IonInput, IonItem, IonLabel, IonList, IonLoading, IonPage, IonRow, IonToast, IonToolbar } from '@ionic/react';
...
export default function LobbySearch(props) {
    ...
    const handleSearch = () => {
        if (!searchString || searchString.length < 3)
            ...
            setShowLoading(true);
            LobbyService.getLobbies(searchString).then(x => { setlobbies(x); setShowLoading(false); x.length > 0 ? setResults(`We found these results with the code: ${searchString}`) : setResults(`We didn't found any results with the code: ${searchString}`) })
        }
    }
    return (
        <IonPage>
                                ...
                                <IonCard key={x.owner + x.lobbyCreatedDate}>
                                    ...
                                    <IonCardContent>
                                        <IonGrid>
                                            <IonRow>
                                                <IonCol size="4">
                                                    <img src={x.image ? x.image : require("../../resources/placeholder.png")} style={{ maxWidth: '250px', height: "120px" }}></img>
                                                </IonCol>
